...
class SpaceInsertTest {
    ...
    fun `insert 2 lines at boundary shifts content below down`() {
        ...
        documentModel.main.activeStrokes.add(strokeOnLine(0))
        documentModel.main.activeStrokes.add(strokeOnLine(1))
        documentModel.main.activeStrokes.add(strokeOnLine(2))

        val originalY0 = documentModel.main.activeStrokes[0].points[0].y
        val originalY1 = documentModel.main.activeStrokes[1].points[0].y
        val originalY2 = documentModel.main.activeStrokes[2].points[0].y

        // Insert 2 lines at boundary between line 0 and line 1
        SpaceInsertMode.insertSpace(documentModel.main, lineSegmenter, anchorLine = 1, linesToInsert = 2)

        // Line 0 should not move
        assertEquals(originalY0, documentModel.main.activeStrokes[0].points[0].y, 0.1f)
        // Lines 1 and 2 should shift down by 2 * LINE_SPACING
        assertEquals(originalY1 + 2 * ls, documentModel.main.activeStrokes[1].points[0].y, 0.1f)
        assertEquals(originalY2 + 2 * ls, documentModel.main.activeStrokes[2].points[0].y, 0.1f)
    }
    ...
    fun `insert at line 0 shifts everything down`() {
        ...
        SpaceInsertMode.insertSpace(documentModel.main, lineSegmenter, anchorLine = 0, linesToInsert = 1)

        assertEquals(originalY0 + ls, documentModel.main.activeStrokes[0].points[0].y, 0.1f)
        assertEquals(originalY1 + ls, documentModel.main.activeStrokes[1].points[0].y, 0.1f)
    }

    @Test
    fun `insert space shifts diagram areas below anchor`() {
        documentModel.main.activeStrokes.add(strokeOnLine(0))
        documentModel.main.diagramAreas.add(DiagramArea(startLineIndex = 2, heightInLines = 3))

        SpaceInsertMode.insertSpace(documentModel.main, lineSegmenter, anchorLine = 1, linesToInsert = 2)

        // Diagram area should shift down by 2
        assertEquals(4, documentModel.main.diagramAreas[0].startLineIndex)
        assertEquals(3, documentModel.main.diagramAreas[0].heightInLines)
    }

    @Test
    fun `insert space inside diagram shifts entire diagram`() {
        documentModel.main.diagramAreas.add(DiagramArea(startLineIndex = 1, heightInLines = 3))
        documentModel.main.activeStrokes.add(strokeOnLine(2))  // inside diagram

        SpaceInsertMode.insertSpace(documentModel.main, lineSegmenter, anchorLine = 2, linesToInsert = 2)

        // Diagram should shift as a unit, not expand
        assertEquals(3, documentModel.main.diagramAreas[0].startLineIndex)
        assertEquals(3, documentModel.main.diagramAreas[0].heightInLines)
    }
    ...
    fun `insert space inside diagram shifts all strokes in diagram`() {
        ...
        documentModel.main.diagramAreas.add(DiagramArea(startLineIndex = 1, heightInLines = 3))
        documentModel.main.activeStrokes.add(strokeOnLine(0))  // outside diagram, above anchor
        documentModel.main.activeStrokes.add(strokeOnLine(1))  // inside diagram, above anchor
        documentModel.main.activeStrokes.add(strokeOnLine(3))  // inside diagram, at/below anchor

        val originalY0 = documentModel.main.activeStrokes[0].points[0].y
        val originalY1 = documentModel.main.activeStrokes[1].points[0].y
        val originalY3 = documentModel.main.activeStrokes[2].points[0].y
        ...
        assertEquals(originalY3 + 2 * ls, documentModel.main.activeStrokes[2].points[0].y, 0.1f)
    }

    @Test
    fun `insert space does not shift diagram areas above anchor`() {
        documentModel.main.diagramAreas.add(DiagramArea(startLineIndex = 0, heightInLines = 2))
        documentModel.main.activeStrokes.add(strokeOnLine(3))

        SpaceInsertMode.insertSpace(documentModel.main, lineSegmenter, anchorLine = 3, linesToInsert = 1)

        // Diagram at line 0 should not move
        assertEquals(0, documentModel.main.diagramAreas[0].startLineIndex)
    }
    ...
    fun `remove empty lines above anchor`() {
        ...
        documentModel.main.activeStrokes.add(strokeOnLine(0))
        documentModel.main.activeStrokes.add(strokeOnLine(4))

        val originalY0 = documentModel.main.activeStrokes[0].points[0].y
        val originalY4 = documentModel.main.activeStrokes[1].points[0].y

        // Anchor at line 4 (where content is), remove 3 empty lines above it
        val removed = SpaceInsertMode.removeSpace(documentModel.main, lineSegmenter, anchorLine = 4, linesToRemove = 3)

        assertEquals(3, removed)
        // Line 0 should not move (above the gap)
        assertEquals(originalY0, documentModel.main.activeStrokes[0].points[0].y, 0.1f)
        // Line 4 should shift up by 3 * LINE_SPACING (now at line 1)
        assertEquals(originalY4 - 3 * ls, documentModel.main.activeStrokes[1].points[0].y, 0.1f)
    }
    ...
    fun `remove space blocks at content above — only removes empty lines`() {
        ...
        val removed = SpaceInsertMode.removeSpace(documentModel.main, lineSegmenter, anchorLine = 4, linesToRemove = 4)

        assertEquals(1, removed)
        // Line 4 content should shift up by 1 (now at line 3)
        val newLine = lineSegmenter.getStrokeLineIndex(documentModel.main.activeStrokes[2])
    ...
    fun `remove space with no empty lines above does nothing`() {
        ...
        val removed = SpaceInsertMode.removeSpace(documentModel.main, lineSegmenter, anchorLine = 2, linesToRemove = 2)
    ...
    fun `remove space shifts diagram areas up`() {
        documentModel.main.diagramAreas.add(DiagramArea(startLineIndex = 5, heightInLines = 2))
        // Lines 2-4 are empty, content on line 0
        documentModel.main.activeStrokes.add(strokeOnLine(0))

        // Anchor at line 5 (diagram), remove 3 empty lines above
        val removed = SpaceInsertMode.removeSpace(documentModel.main, lineSegmenter, anchorLine = 5, linesToRemove = 3)

        assertEquals(3, removed)
        assertEquals(2, documentModel.main.diagramAreas[0].startLineIndex)
    }
    ...
    fun `remove space with anchor inside diagram shifts containing diagram`() {
        ...
        documentModel.main.activeStrokes.add(strokeOnLine(0))
        documentModel.main.activeStrokes.add(strokeOnLine(4))  // inside diagram
        ...
        assertEquals(1, documentModel.main.diagramAreas[0].startLineIndex)
        assertEquals(3, documentModel.main.diagramAreas[0].heightInLines)  // height unchanged
        // Stroke inside diagram also shifted
        val strokeLine = lineSegmenter.getStrokeLineIndex(documentModel.main.activeStrokes[1])
    ...
    fun `countEmptyLinesAbove counts correctly`() {
        documentModel.main.activeStrokes.add(strokeOnLine(0))
        // Lines 1-4 are empty
        documentModel.main.activeStrokes.add(strokeOnLine(5))

        val count = SpaceInsertMode.countEmptyLinesAbove(documentModel.main, lineSegmenter, anchorLine = 5)
    ...
    fun `countEmptyLinesAbove stops at content`() {
        ...
        documentModel.main.activeStrokes.add(strokeOnLine(3))
        // Lines 4-5 empty
        documentModel.main.activeStrokes.add(strokeOnLine(6))

        val count = SpaceInsertMode.countEmptyLinesAbove(documentModel.main, lineSegmenter, anchorLine = 6)
        assertEquals(2, count)  // lines 4-5 only
    }

    @Test
    fun `countEmptyLinesAbove stops at diagram area`() {
        documentModel.main.diagramAreas.add(DiagramArea(startLineIndex = 1, heightInLines = 2))
        // Lines 3-4 empty
        documentModel.main.activeStrokes.add(strokeOnLine(5))
    ...
    fun `insert space then undo restores original positions`() {
        ...
        documentModel.main.activeStrokes.addAll(restored.strokes)

        assertEquals(originalY0, documentModel.main.activeStrokes[0].points[0].y, 0.1f)
        assertEquals(originalY1, documentModel.main.activeStrokes[1].points[0].y, 0.1f)
    }
}
