...
class RecognizedTextView @JvmOverloads constructor(
    ...
    companion object {
        ...
        private const val BOTTOM_PADDING   = 0f
    }
    ...
    private fun rebuildRenderItems(paragraphs: List<List<TextSegment>>, diagrams: List<DiagramDisplay>) {
        ...
        val diagramItems = diagrams.map { diagram ->
            ...
            val isPartial = diagram.visibleHeightPx < diagram.heightPx
            val fullRenderedHeight = diagram.heightPx * scale + PARAGRAPH_SPACING
            // No spacing after partial diagrams so content is flush with the divider
            val renderedHeight = diagram.visibleHeightPx * scale + if (isPartial) 0f else PARAGRAPH_SPACING
            Indexed(
                lineIndex = diagram.startLineIndex,
                item = DiagramRenderItem(diagram.strokes, scale, diagram.offsetY, fullRenderedHeight, renderedHeight, diagram.startLineIndex),
        ...
        if (items.isNotEmpty()) {
            ...
            when (last) {
                is TextRenderItem -> {
                    val trimmed = last.copy(heightPx = last.layout.height.toFloat())
                    items[items.lastIndex] = trimmed
                }
                is DiagramRenderItem -> {
                    val trimmed = last.copy(heightPx = last.heightPx.coerceAtMost(
                        last.fullHeightPx - PARAGRAPH_SPACING
                    ))
                    items[items.lastIndex] = trimmed
                }
            }
